...
not across one, so swapping the LM without re-tuning silently converts a
+1.65 component into a −0.14 one. What authority amplifies is not the model's
...
- Second-pass signals must add *evidence the first pass lacks*, never re-apply
  ...
  *into* the search, where it steers which hypotheses survive, the same
  ...
  lands 0.85 under it, in the mid- and tail-frequency words, so #34's
