...
| 60 | the browser beam's width claim, re-tested through a reproduced export — `export_web_decoder.py` (the original export died with its session; K is baked into the step graph's batch dim, so the in-page claim was unfalsifiable) — plus a teacher-forced rescore of the analytic engine's top-40 through the same K=8 graph, on 522 real trackpad swipes | **"8/16/32/64 identical" is false both ways on the grown corpus**: 14 swipes lose words the model teacher-forces into the top 3 (four to rank 1 — weak opening letters, strong tails), K=16 recovers 21 but its extra candidates push 11 accepted words off the top-3 bar; the rescore recovers 18 and loses zero (AR word scores are exact — one token path per word — so added candidates only extend the list). Shipped in the game: 71.6 → 75.1 top-3. Export parity trap worth keeping: the legacy exporter leaves the model in train mode, and a parity reference taken after export runs with dropout live | `export_web_decoder.py`, swipepad `web/ar-decode.js` (arRescue) |
