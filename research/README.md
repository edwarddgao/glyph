...
|---|---|---|---|---|---|---|
| *(no LM)* | — | — | 92.53 | — | — | — |
| distilgpt2 | 82M | 2019 | 93.73 | 94.24 | 94.73 | 43% |
...
*distilgpt2 row added 2026-09-04 for the on-device keyboard (`keyboard/`),
same command with `--lm distilgpt2`: paired against gpt2 it ties at joint
(+0.07, p=0.7) and streaming (−0.15, p=0.29) but loses at lookahead-1
(−0.32, 39 fixed / 63 broken, p=0.022) — the one commitment policy a
keyboard actually ships. 34% of the headroom at lookahead-1 to gpt2's 40%.*

*Sentence-initial words, added 2026-09-04 from the saved hypotheses of this
slice (795 first words of 7,539): the LM contributes nothing at position 0
in streaming mode — no-LM 90.8% → distilgpt2 90.7%, gpt2 90.6% — while
later words move 92.0 → 94.1/94.3. The delta form's only left-context
evidence at the first word is `log P(w | BOS) − marginal(w)`, and at μ=0.8
it is a wash. Lookahead-1 (91.4/91.2) and joint (91.8/91.4) recover
+0.6–1.0 there, all of it right context. Nothing in the stack was ever
tuned for the first word specifically (a separate μ at position 0, or a
sentence-start-aware prior) — an open item, and the first word is the one
...
| 85 | pre-ship lever audit on the phone's cross-domain evidence (543 real-iPhone words + 1,337 FUTO replay words, everything paired McNemar against the shipped `ar_mixed_s1` stack): every AR checkpoint in the run library as first pass (conformer, conformer s2, clean, n128, d192, d256, cont6, perm25, ar_full+MMI) plus one MMI epoch on the shipped encoder; the sentence stage's knobs (list depth 8/16/24, μ 0.6–1.2, a separate first-word μ, α, λ, sentence beam 4/16); seven LMs on the shipped lists (gpt2 → medium as drop-ins, SmolLM2-135M/360M, Qwen3-0.6B, Qwen3.5-0.8B); touch offset removal; time scaling ×0.5/1.5/2; and per-user fine-tuning, two-fold by sentence, with and without FUTO replay | **nothing this evidence can see: the shipped configuration is at or within noise of the best cell in every grid.** No encoder beats `ar_mixed_s1` on the iPhone words (conformer ties 72.6 vs 73.3, best tail 67.4, worse everyday); MMI on it is +0.7 first pass (11/7, p=0.48) and the fused stage absorbs it (77.9 vs 78.5, 5/8). Deeper lists move nothing though truth-in-beam is 93.6 vs 89.1 in top-8 — the 9th–64th words are not LM-rescuable; λ 0.25 is the one setting the set certifies (λ 0: −1.7, p=0.04); first-word μ 0.4 is +4.2 on the iPhone first word and +0.6 on FUTO's, n.s., wash overall. LMs: gpt2/medium +0.3/+0.5, n.s.; **SmolLM2-360M +1.2 on FUTO (21/4, p<0.001) at gpt2-medium's size, level on iPhone (17/17)** — 2024 text, not parameters; Qwen bases lose 2–3 on the iPhone set, mostly the first word (no BOS). Offset: the iPhone finger lands +0.15/+0.29 key widths right of centre, removing it does nothing (21/21) and costs FUTO 1.4; time-scaling hurts monotonically (t×2: −4.1, p<0.001). Adaptation on ~270 own swipes: +0.9, n.s., either arm; without replay FUTO −2.3. The one open lever is a modern-text LM port, post-launch | `eval_phone_levers.py`, `probe_user_adapt.py`, `runs/ar_mixed_mmi/`, iphone/README "Pre-ship lever audit" |
...
- Training-seed noise at the beam is ~0.02 in-domain and <0.2 cross-corpus
  (#74), so beam-level deltas of a few tenths in this log are real; greedy
  deltas under ~0.5 are not. Encoder capacity from 1M to 15M parameters
  moves the beam +0.34 and the ceiling not at all (#75) — the acoustic side
  is neither capacity- nor budget-limited (#17) *under this data*, and at
  matched size the trunk family is worth ~+0.2 at most: a conformer trunk
  is +0.16 in-domain across three runs, replicated but small; self-attention
  appended to the TCN, a dual time/arclength stream, and doubled frames are
  nulls or losses (#83). The
  residual is now decomposed (#76): ~1.25 pts out-of-lexicon, ~1.2 pts
  head confusions where, on every recurring pair tested, dedicated
  pairwise classifiers fail on the same gestures (no acoustic signal
  left to learn; context and the user remain), and ~3.0 pts rare in-lexicon
  words with ≤50 training gestures — 10% of val words carry 55% of the
  errors. ~0.2–0.6 pts of the val error rate is label noise — donors
  swiping a different word than the prompt, mostly alignment slips (#79)
  — so the stack's clean-label ceiling sits that much above every number
  here; on How We Swipe the same check finds ~3–4 pts of label mismatch
  (foreign words, aborted gestures, swiped misspellings under the prompt
  label), a quarter to a third of the cross-corpus gap (#80); #8's
  "intrinsic" 5.5 re-reads as 1.8 once the eval set is full-size and
  label-filtered (#81). Training on the filtered FUTO corpus is a null
  in-domain and +1.0 on hws (#82): `futo_clean/train` is the training
  set for any cross-corpus claim from here on. The
  remaining acoustic lever is per-word tail coverage. Its
  ceiling is now measured (#77): ~+0.25 cross-corpus, ~0 in-domain,
  whether harvested by a dosed fine-tune (#62b) or a tax-free gated
  two-expert decode. #60's "head tax" was forgetting from a 25%-of-
  donors replay plus continued-training drift, not a prior — and the
  control it lacked (six more real-only epochs) costs −0.4 on hws by
  itself. Any further fine-tune of the frozen encoder must carry that
  control.
- The encoder's implicit prior is the cheapest cross-corpus lever left:
  subtracting a quarter of its internal-LM estimate and raising the
  external unigram to compensate is +0.6 on hws through the full
  stack, replicated on a disjoint slice, and a null in-domain (#78).
  Recommended cross-corpus fused config: `--alpha 0.6 --lam 0.25
  --mode mean`; in-domain the deployed α=0.4, λ=0 stands.
...
## The post-encoder stack on modern text

```bash
python scripts/fetch_text_domains.py --n 1500         # tweets, reddit, wildchat (+ dialog, movies)
python scripts/eval_text_domain.py --domains tweets,reddit,wildchat --n 500 --real-futo
```

Both swipe corpora are prompted transcription: FUTO's text is Common Voice
(Wikipedia-style read speech, ~10 words) and How We Swipe's prompts are
four-word bags with brand names. Neither is messaging. Since the stack after
the encoder — lexicon, LM, commitment policy — depends on the text and not
the gestures, it can be tested on modern conversational text with the #57
domain-randomized min-jerk generator supplying the gestures. 2026-09-04,
~450–490 sentences per domain (3–12 letters-only words, all in the lexicon),
runs/full + trie beam top-8 + distilgpt2 fused search:

| text | OOV | first pass | streaming | lookahead-1 | joint | ceiling@8 | first word: no LM → look-1 |
|---|---|---|---|---|---|---|---|
| futo/validation, real gestures | 1.4% | 94.2 | 95.2 | 95.8 | 96.2 | 99.0 | 95.5 → 95.2 |
| futo/validation, synthetic | 1.4% | 86.0 | 88.4 | 89.5 | 90.3 | 95.0 | 89.1 → 90.0 |
| tweets (tweet_eval, 2017–20) | 0.7% | 78.3 | 81.2 | 83.9 | 85.0 | 93.3 | 69.9 → 76.2 |
| reddit comments | 0.4% | 80.2 | 82.6 | 86.0 | 87.6 | 94.6 | 67.9 → 75.9 |
| WildChat user turns (2023–24) | 1.6% | 82.1 | 84.6 | 86.6 | 87.7 | 93.9 | 81.0 → 84.1 |

Three readings. (1) The LM converts the same share of the n-best headroom
on every domain — 34–40% at lookahead-1 — so the gpt2-family gain measured
on Common Voice carries to messaging text; in absolute terms it is worth
more there (+5.6/+5.8 on tweets/reddit vs +1.6 on real FUTO) because the
first pass has more headroom on casual vocabulary. (2) **The sentence-initial
word is the weak spot of casual text and the LM does help it there**: no-LM
first-word accuracy is 68–70% on tweets/reddit against ~80% for later
words, and lookahead-1 recovers +6–8 points of it — the "words that don't
start sentences" signal the FUTO measurement (above, 90.8 → 90.7) could not
show because Common Voice sentences start like Wikipedia sentences. (3)
Lexicon coverage is not the problem on these domains (0.4–1.6% OOV after
normalization; the misses are usernames, lorem ipsum, product names), the
first pass is: 78–82 on synthetic gestures for casual words vs 86 for
Common Voice words under the identical generator. Synthetic gestures cost
~8 points of first pass on the FUTO anchor (86.0 vs 94.2 real), so the
absolute rows are not comparable to real-gesture numbers; the domain
*differences* under one generator are.

Not fetched: DailyDialog and Cornell movie dialogue (script-based loaders
that stall under datasets 5.x); three real-text domains were enough for the
question.

### Is the scoring algebra robust across registers?

```bash
python scripts/eval_scoring_forms.py --n 300 --domains tweets,reddit,wildchat
```

The keyboard benchmark (iphone/README) put the shipped stack 4.6 behind
QuickPath on the user's own gestures, two thirds of it words that sat in
our top-8 and lost the ranking (chai→chap, bro→bo, ghosted→hosted). The
obvious suspect was the hand-fitted score — ctc + α·uni + β·len +
μ·(LM − marginal), three weights fitted on Common Voice — so four forms
were fitted once on 300 real FUTO sentences (grid, lookahead-1) and then
held fixed on every other domain, recording how much each domain's own
optimum would add:

| form | futo real | futo synth | tweets | reddit | wildchat | capture (real) | mean gap to own best |
|---|---|---|---|---|---|---|---|
| first pass, no LM | 94.5 | 86.1 | 78.8 | 82.8 | 83.9 | 68.0 | — |
| **current** ctc+0.8·uni+1.2·len+0.8·Δ | **95.8** | **89.5** | **83.9** | **89.3** | **87.8** | **71.7** | **0.28** |
| no unigram, Δ-LM | 95.2 | 85.7 | 72.9 | 80.4 | 78.5 | 63.5 | 0.95 |
| shallow fusion (raw LM owns the prior) | 95.6 | 89.0 | 82.6 | 88.0 | 87.2 | 71.1 | 0.77 |
| ILM-subtracted + raw LM (ASR form) | 95.5 | 88.7 | 81.5 | 87.2 | 86.5 | 70.2 | 1.24 |

**The shipped algebra is the robust one.** Its Common-Voice weights are
within 0.3–0.7 of every domain's own optimum, including the iPhone capture
gestures (+0.7), and it is best on all six columns. The external unigram is
load-bearing exactly where it was suspected of doing harm: removing it costs
11 points on tweets and 8 on the capture set, and the raw-LM forms that let
the LM own the prior are both a little worse and more domain-sensitive.
The ranking losses to QuickPath are therefore not a weight-tuning problem;
the score is fine, the components under it do not know the register (the
LM) or the device (the encoder: first pass 68.0 on real capture gestures
against 94.5 on real FUTO ones — the acoustic gap dwarfs everything the LM
recovers, +3.7). What would move the capture column is data on both sides:
iPhone gestures for the encoder (per-user adaptation, #58) and
messaging-scale text for the LM, each judged by this same fixed-weight,
all-domains test.

## Which encoder generalizes to a real iPhone?

The keyboard shipped `runs/full`, the capture study's encoder. On the only
real-iPhone gestures in hand (the 543 capture-study swipes: 2× faster and 2×
sloppier than FUTO's donors, 30% with a start or end more than two key radii
off), first pass with the trie beam, best α per model, β=1.2:

| encoder | top-1 | in top-8 | in beam-64 | everyday | tail | sloppy | tidy |
|---|---|---|---|---|---|---|---|
| CTC `runs/full` (shipped) | 67.8 | 82.5 | 85.5 | 75.9 | 60.3 | 56.1 | 72.8 |
| CTC `runs/mmi` | 67.8 | 84.0 | 85.6 | 76.2 | 59.9 | 59.1 | 71.5 |
| CTC `runs/perm25mmi` | 70.0 | 84.9 | 87.5 | 78.2 | 62.4 | 59.8 | 74.4 |
| AR `ar_full` | 68.3 | 85.8 | 92.3 | 75.9 | 61.3 | 57.3 | 73.1 |
| AR `ar_clean_s1` (#82a) | 70.9 | 86.7 | 92.8 | 80.8 | 61.7 | 59.8 | 75.7 |
| AR `ar_mixed_s1` (#82b) | 71.8 | **88.2** | **93.6** | 80.1 | **64.2** | 59.1 | 77.3 |
| AR freeze five (clean + MMI) | **72.4** | 87.8 | 92.4 | **81.2** | **64.2** | 59.8 | **77.8** |
| AR `ar_ft10` (synthetic pretrain + 10% real) | 64.5 | 86.0 | 91.3 | 72.8 | 56.7 | 49.4 | 71.0 |

The cross-corpus levers the log found on How We Swipe transfer to the
iPhone: freeze five is +4.6 top-1 over the shipped encoder and the AR family
puts the truth in the beam on 92–94% of swipes against CTC's 85.5 — that
coverage is what the LM stage feeds on. Mixing in the second real population
(#82b) buys the best coverage and the best tail; cleaning + MMI the best
top-1. The sloppy 30% stay at ~59 for every model: no training-data lever
in the log reaches gestures whose endpoints are two keys off, which is the
per-user-adaptation regime (#58). Synthetic pretraining with little real
data (ar_ft10) generalizes worst — the opposite of its in-domain rank.

With the shipped LM stage on top (distilgpt2, delta form, lookahead-1, top-8
lists), on the same 543 gestures — QuickPath replayed on them scored 74.9 /
82.0 everyday / 68.3 tail / 69.5 first word:

| encoder → fused lookahead-1 | overall | everyday | tail | first word |
|---|---|---|---|---|
| CTC `runs/full` (shipped until 09-04) | 71.6 | 82.4 | 61.7 | 59.4 |
| AR freeze five, α=0.6 | 75.9 | 87.0 | 65.6 | 61.5 |
| AR `ar_mixed_s1`, α=0.4 | **77.0** | **87.0** | **67.7** | **64.6** |

The mixed encoder's coverage converts: +5.4 over the shipped stack, and
above QuickPath's replayed 74.9 on this user's gestures, with the tail
within 0.6 of it and the first word still 5 behind. Offline number against
a replayed one — the paired test is the replay benchmark once the AR beam
runs on the phone. Shipping consequence: the keyboard should run the AR
decoder (an AR beam with per-step Core ML calls), and the training set for
"anyone on iPhone" should include every real population available.

*Done the same day: `ar_mixed_s1` ported (`keyboard/`: two Core ML models,
Swift AR beam exact against `ar_beam` on 24 goldens, beam 32 — costs 0.6
fused on the iPhone set vs 64 for half the step time — ranking α=0.6 β=1.2
λ=0.25 with the ILM table precomputed per lexicon word; fitted on FUTO:
α/λ grid gave 96.4–96.9 on 300 real FUTO sentences with λ=0.25 best at every
α, and 76.4–78.5 on the iPhone set, λ=0.25 +0.7–1.9 there). **Replay
benchmark, byte-identical gestures on the simulator: Swipe 77.9 vs QuickPath
74.9 on the 542 iPhone words (+3.0, p=0.09; everyday +6.9 p=0.001; tail and
first word level), 93.4 vs 90.2 on 1,337 FUTO words (p<0.001)** — the offline
77.0 reproduced through the phone stack (77.9), and the CTC build's −4.6
became +3.0 with only the encoder changed. Gboard and SwiftKey, replayed on the
phone itself: Gboard 68.5 / 88.0, SwiftKey 69.0 / 85.9 — both behind
QuickPath, and 9.4 / 5.5 and 8.9 / 7.6 behind Swipe (p<0.001 each).
Full table in `iphone/README.md`.*
...
scripts/
  ...
  eval_ar_decoder.py      greedy vs trie-constrained AR beam; --save-preds for paired comparison
  ...
  fetch_text_domains.py   modern conversational text samples (tweets, reddit, wildchat, …)
  eval_text_domain.py     the post-encoder stack on those texts with synthetic gestures
  eval_scoring_forms.py   fused-score forms fitted on FUTO, held fixed across registers
  eval_phone_levers.py    pre-ship audit on the replay sets: checkpoints, sentence-stage knobs, LMs, offset, speed (#85)
  probe_user_adapt.py     per-user fine-tuning ceiling, two-fold over the real-iPhone gestures (#85)
```
